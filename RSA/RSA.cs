...
namespace RSA
{
    public static class RSA
    {
        ...
        public static string Sign(string privKey, string msgToSing)
        {
            var secret = Network.Main.CreateBitcoinSecret(privKey);
            var signature = secret.PrivateKey.SignMessage(msgToSing);

            var v = secret.PubKey.VerifyMessage(msgToSing, signature);
            return signature;
        }
